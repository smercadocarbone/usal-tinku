...
### `certificados_antecedentes_penales` _(agregado, enmienda Constitución v2.1)_
...
### 2.4 Carga y vencimiento del CAP (US-6) _(agregado, enmienda Constitución v2.1)_
...
| `POST`   | `/api/tutores/antecedentes-penales`               | Carga del CAP (respeta backoff, igual patrón que credenciales).                                                                           |
| `GET`    | `/api/admin/moderacion/antecedentes-penales`      | Cola de M8 — pendientes y `en_revision_legal`.                                                                                            |
| `PATCH`  | `/api/admin/moderacion/antecedentes-penales/{id}` | Aprobar / rechazar (BR-CAP-01) / marcar `en_revision_legal` (BR-CAP-02), con `categoria_antecedente` en el body para trazabilidad.        |
