...
3. Protocolo: test RED primero (guardá la salida del fallo para el reporte), fix mínimo, y suite completa con: `cd backend && JAVA_HOME=/Library/Java/JavaVirtualMachines/temurin-21.jdk/Contents/Home ./mvnw -B -o test` (con `-o`, offline: la conexión a internet es mala y todas las dependencias ya están en ~/.m2). Baseline de hoy: 451 tests, 0 failures (AGENTS.md dice 426: está desactualizado). N nunca puede bajar de 451.
...
tareas = {
 ...
 "fase2-evidencia-upload": dict(branch="aud/fase2-evidencia-upload", spec="docs/superpowers/specs/remediacion/FASE2-09-evidencia-upload.md",
   ...
   extra_regla="Baseline de esta corrida: 442 tests (FASE2-04 ya está en main). La retención del clip (§3 de la spec) exige verificar BR-KS-02 antes de agregar la fila a la Tabla de Tiempos: si no lo fija explícitamente, PARÁ."),
