...
| FR-SEC-004 | Alertas del kill-switch: track independiente, prioridad alta, revisión del Admin de Moderación y Seguridad en 12hs sin descargo previo obligatorio; suspensión preventiva ya automática desde M3. Resolver la Alerta (`reactivar` o `sancionar`) publica `alerta.resuelta`, que reembolsa el escrow que el kill-switch dejó en pausa (ADR-M3-02). |
