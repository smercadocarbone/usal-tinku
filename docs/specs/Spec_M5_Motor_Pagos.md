...
| `sesion.killswitch_menor` | M3 | Pausa el escrow (`pausado_denuncia`); el reembolso completo espera a `alerta.resuelta` (ADR-M3-02, AUD-005). |
| `sesion.killswitch_adultos` | M3 | Pausa el escrow; el reembolso completo espera a `alerta.resuelta`, y se hace incluso si el Estudiante fue el detectado — ver FR-PAG-012 y ADR-M3-02. |
| `alerta.resuelta` _(agregado, ADR-M3-02, 2026-09-22)_ | M9 | Reembolso completo al Estudiante del escrow que pausó el kill-switch, sea la decisión `reactivar` o `sancionar`. No-op si la transacción ya no está en `pausado_denuncia`. Payload: `reservaId`. |
| `denuncia.registrada` (con escrow activo) | M9 | Pausa la liberación de fondos hasta resolución. |
| `denuncia.resuelta` | M9 | Reanuda la liberación o ejecuta el reembolso/sanción que corresponda, según `resolucion` (`infundada` → reconta las 24hs de FR-PAG-002; `fundada` → liberación al Tutor de la sesión puntual si el encadenamiento de la sanción de M4 no la canceló antes; `escalada` → reembolso completo FR-PAG-011/009). **Payload enriquecido por T-M9-04** (documentado en `DenunciaResueltaEvent`): agregó `reservaId` (nullable, denuncia sin sesión) y `resolucion`; conserva el constructor original (stub → `reservaId=null`/`resolucion=null`) para compatibilidad del listener de M4, que ante `resolucion=null` mantiene el comportamiento de cancelar reservas futuras. |
