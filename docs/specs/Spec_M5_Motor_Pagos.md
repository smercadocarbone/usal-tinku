...
| FR-PAG-018 _(agregado 2026-09-25, R3, ADR-M5-02)_ | Con la app de marketplace configurada, cada Tutor conecta su MercadoPago por OAuth (state de un solo uso + PKCE; tokens cifrados AES-256-GCM) y toda operación de cobro usa el token del Tutor de la Reserva, así el `marketplace_fee` reparte el cobro. Sin cuenta `CONECTADA`, el Tutor no aparece en el matching ni se le puede reservar (422). Los tokens se renuevan a diario si vencen en menos de 30 días; si falla, la cuenta queda `ERROR` y se avisa al Tutor. No se puede desconectar con reservas pendientes/futuras o escrows en la ventana (409). El escrow de 24 hs pasa a ser una ventana lógica de reembolso. |
