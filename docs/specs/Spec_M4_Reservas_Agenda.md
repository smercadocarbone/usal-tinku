...
- **Dado** que falten menos de 30 minutos para el horario elegido (eran 15 hasta 2026-09-26), **cuando** se intente reservar, **entonces** el sistema lo bloquea (FR-RES-013). Con 30, aunque el pago tarde sus 15 minutos, el Tutor se entera con margen para prepararse.
...
- **Dado** que falten menos de 24 horas _(enmendado 2026-09-26)_, **cuando** se reprograme a otro horario disponible del Tutor, **entonces** se aplica igual que con 24 hs o más, conservando el pago, hasta 1 hora antes de la clase (el doble de la ventana mínima: el horario liberado queda abierto para que otra persona lo reserve). Sin límite de cambios (FR-RES-016).
- **Dado** que falte menos de 1 hora, **cuando** se intente reprogramar, **entonces** se rechaza con un mensaje claro y la Reserva sigue en pie — nunca se cancela sola; si no puede asistir, quien pagó la cancela (US-7).
...
| FR-RES-007              | Prevención de reservas superpuestas por **rango** `[horario, horario_fin)` del Tutor y del beneficiario (las contiguas son válidas; las canceladas no cuentan). _(Precisado en FASE2-01/AUD-009.)_                                                                                                                                                                                                                                                                |
...
| FR-RES-013              | Bloqueo de reservas con menos de 30 minutos de anticipación (eran 15 hasta 2026-09-26).                                                                                                                                                                                                                                        |
| FR-RES-015              | Reprogramación con ≥24hs conserva el escrow y el precio original, sin nueva transacción.                                                                                                                                                                                                            |
| FR-RES-016              | _(enmendado 2026-09-26)_ Reprogramación con <24hs: permitida hasta 1 hora antes de la clase (doble de la ventana mínima), conservando escrow y precio, sin límite de cambios; con menos de 1 hora se rechaza (nunca se cancela sola).                                                                                                                                                                                                                                          |
...
| FR-RES-023 _(enmendado FASE2-01, D6)_ | La duración de la Sesión la elige quien reserva (o el menor en su Solicitud) entre 30 y 180 minutos en bloques de 30, y se congela en la Reserva (`duracion_minutos`, `horario_fin`) — no cambia si el Tutor modifica o borra sus franjas. La franja es un contenedor de bloques: la reserva tiene que entrar entera en una franja activa y empezar alineada a 30 minutos desde su inicio. La reprogramación conserva la duración. |
...
| FR-RES-025 _(agregado 2026-09-25, R1)_ | Solo se reserva (directa, por Solicitud o al reprogramar) a un usuario TUTOR con cuenta ACTIVA; si no, 404 sin filtrar el estado. El Tutor no puede ser el pagador ni el beneficiario de su propia Reserva, ni darle clase a un menor del que es Adulto Responsable → 422. La base lo refuerza con `ck_reservas_tutor_no_es_parte` (V37). |
