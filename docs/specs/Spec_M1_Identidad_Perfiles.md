...
| FR-ID-033 _(2026-09-26, UX Mi cuenta)_ | (a) El backend exige `aceptaTerminos: true` para crear la cuenta (adulto y Tutor) y registra la aceptación de los Términos y de la grabación de solo audio del resumen, una sola vez (ADR-M3-05); refuerza FR-ID-031. (b) El Tutor elige si da clases a menores (`acepta_menores`, V41, por defecto sí): con "no" no le aparece a ningún menor, no se lo puede autorizar y se cancelan sus clases futuras con menores (reembolso total y aviso al AR). Estar habilitado para menores = esta preferencia **y** el CAP vigente (FR-ID-026). (c) Todo el perfil del Tutor vive en Mi cuenta: Presentación (con vista previa), Materias, Precio, Credenciales y Clases con menores. |
