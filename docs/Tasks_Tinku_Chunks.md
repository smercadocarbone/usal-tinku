...
- [x] **Chunk TESIS-RESUMEN** — Proveedor LLM + grabación de solo audio + adicional pago (T-TES-07, T-TES-08, T-TES-09; specs `tesis/T07`, `T08`, `T09`) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25. GPT-4o (ADR-M6-03); grabación en el navegador del Tutor en vez de Egress (ADR-M3-04, Constitución v2.4); adicional de $770 con reembolso parcial (BR-PAG-11). AUD-024 CERRADO. Pendiente fuera del código: el texto legal de la cláusula (hasta entonces el adicional queda apagado en prod)._
- [x] **Chunk UX-TYC + FOTO** — Puntos clave de los Términos en el registro (T-UX-TYC, FR-ID-031) y foto obligatoria del Tutor (T-ID-FOTO, FR-ID-028) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25._
- [x] **Chunk ROL-R1** — Seguridad de roles y guardas de Admin (T-ROL-R1, FR-ID-032, FR-RES-025, migración V37) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25._
- [x] **Chunk PAG-R2** — Conciliación de pagos (T-PAG-R2, FR-PAG-017, migración V38) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25._
- [x] **Chunk PAG-R3** — Modelo de cobro A, OAuth por Tutor (T-PAG-R3, ADR-M5-02, FR-PAG-018, migración V39) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25. Backend; la UI sale con PAG-R5._
- [x] **Chunk PAG-R4** — Reembolso del adicional: outbox + cola de Soporte (T-PAG-R4, BR-PAG-11, migración V40) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25._
- [x] **Chunk PAG-R5** — Mis cobros del Tutor + vista del AR por hijo + 409 con código (T-PAG-R5; cierra T-M4-13/14/15) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25._
- [x] **Chunk UX-CUENTA** — Mi cuenta unificada del Tutor + consentimiento único + selector de fecha (T-UX-CUENTA, ADR-M3-05, migración V41) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-26, PR aparte del de revisión por rol._
- [x] **Chunk M1-OCR-FIX** — OCR del DNI por programa `tesseract` + 6 intentos de foto por ciclo (T-M1-OCR-FIX, ADR-M1-08) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-26. Suite backend 591/0/0._
- [x] **Chunk M2-REV** — Revisión del matching: "no confiable" para menores, reputación que solo desempata, mínimo de relevancia, filtro por nivel y score por tema (T-M2-REV, ADR-M2-02, ADR-M2-03, V42) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-26. Suite backend 599/0/0._
- [x] **Chunk M2-AREA** — Catálogo vivo: recomendación por área + temas sugeridos agregados para el Admin (T-M2-AREA, ADR-M2-04, V43) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-26. Suite backend 607/0/0._
- [x] **Chunk M2-AREA-FIX** — nivel escrito, área por palabras, textos naturales en /buscar (T-M2-AREA-FIX, ADR-M2-04 enmendado) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-26._
- [x] **Chunk M4-PLAZOS** — reservar hasta 30 min antes; cambio de horario con <24 hs hasta 1 h antes (T-M4-PLAZOS) — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-26._
- [ ] **Chunk ESPECIALIDADES** — Especialidades con insignia por especialidad y verificación de credenciales contra registro oficial (T-ESP-01..04, ADR-M1-06). _Pendiente de implementar; falta la cláusula de consentimiento._
...
- [~] **Chunk TESIS-DEPLOY** — Producción en Coolify (T-TES-13, spec `tesis/T13-despliegue-produccion.md` + ADR-000-07) — _branch `claude/lucid-lovelace-htlv4z`. Parte A hecha 2026-09-24; Parte B (cuentas, DNS, secretos, ensayo) en el runbook._
...
- [x] **Chunk FASE3-resto + FASE2-10/09 + FASE4 parcial** — _branch `claude/lucid-lovelace-htlv4z`, 2026-09-25: AUD-021 (evidencia por upload + purga BR-KS-02), AUD-031 (smoke E2E nocturno; primera corrida verde 2026-09-25, CERRADO), 3.1–3.3 (AUD-019, AUD-022), 3.4/3.5 (AUD-025 con V34, AUD-023), 3.7 (AUD-026), 3.8 (AUD-028, D9), 3.10 (AUD-032, Spring Boot 3.5.16), 3.11 (AUD-016), FASE 4: 4.2, 4.4, 4.6. Arreglos de producción de la demo: perfil público con materias de los temas + recompute de embeddings al cambiar temas; vuelta desde MercadoPago (back_urls) sin cobro doble; webhook de MP con `data.id` del cuerpo. Quedan 4.1 y 4.3 (opcionales) y AUD-024/AUD-035 (dependen de ADR/T02)._
- [x] **Chunk FASE3-03** — JWT con UUID como `sub` y `credentials_version` (AUD-027, T-AUD-029; limpieza de AUD-003) — _branch `claude/lucid-lovelace-htlv4z` (adelantado antes del piloto). Cerrado 2026-09-24: V33, claim `cv`, invalidación al cambiar/resetear la contraseña, principal = UUID, LiveKit sin DNI, `/cuenta` sin DNI. T-AUD-023 (ADR del DNI como `sub`) ya no hace falta. Suite 522/0; E2E 79/79._
...
| M6-D (proveedor LLM real)             | Cerrado (ADR-M6-03 + ADR-M3-04)                     | GPT-4o + transcript del audio grabado en el navegador del Tutor; solo con el adicional contratado. |
