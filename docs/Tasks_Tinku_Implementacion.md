...
- [ ] F-04: Build y lint del frontend verdes + flujo manual registro → login → cuenta contra el backend dev. — _Build + lint verdes y endpoints de error verificados contra el backend dev (401 login, 409 registro). El happy path completo queda pendiente de ambiente: el OCR stub dev (`StubOcrService`) fija el DNI a `00000000`, que ya está registrado en la BD local → no se puede crear una cuenta nueva para probar el 201→login→cuenta sin limpiar esa fila/BD._
