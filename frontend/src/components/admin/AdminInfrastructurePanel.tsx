...
function IndicadorEstado({ nivel }: { nivel: ServiceStatusLevel }) {
  ...
  return (
    <span className="inline-flex shrink-0 items-center gap-1.5">
      ...
      <span className="text-xs font-medium text-gray-600">{meta.nombre}</span>
    </span>
  );
}
...
function TarjetaServicio({
  ...
  return (
    <article className="flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-6">
      <header className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-semibold text-gray-900">{estado.name}</h3>
        </div>
        <IndicadorEstado nivel={estado.status} />
      </header>
      <footer className="mt-auto flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500">
        <span>
        ...
        {(healthData.isTestMode || healthData.hasSeedData) && (
          <div className="flex flex-wrap gap-3">
            ...
            {healthData.hasSeedData && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-purple-100 px-3 py-1.5 text-xs font-semibold text-purple-800">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`relative inline-flex h-7 w-12 shrink-0 cursor-pointer items-center rounded-full transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${pagosHabilitados ? "bg-teal-600" : "bg-gray-300"}`}
          >
            <span
              aria-hidden
        ...
        {!pagosHabilitados && (
          <div
            className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800"
            role="alert"
          >
